pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE_NAME = "${env.IMAGE_NAME}:${params.ENV}-${params.GIT_BRANCH}"
        ECS_IMAGE_NAME = "${env.IMAGE_NAME}:${params.IMAGE_TAG}"
        ...
        GIT_CREDENTIAL = "3cf0edc8-9c1d-4ea3-b47c-ddf2350591d5"
    ...
    stages {
            ...
            steps {
                script {
                    sh """
                        ...
                        docker tag ${env.DOCKER_IMAGE_NAME} ${env.ECS_IMAGE_NAME}
        ...
        stage('Registering New Task') {
            steps {
                script {
                    sh """
                        NEW_DOCKER_IMAGE="${env.DOCKER_IMAGE_NAME}"
